...
Telegram::Bot::Client.run(token) do |bot|
  bot.listen do |message|
    ...
    when 'First Group'
    	bot.api.send_message(chat_id: message.chat.id, text: "#{funcToday($firstGroup)}", reply_markup: $daySelect)
    when 'Second Group'
    	bot.api.send_message(chat_id: message.chat.id, text: "#{funcToday($secondGroup)}", reply_markup: $daySelect)
    when 'Tomorrow'
    	funcTomorrow
